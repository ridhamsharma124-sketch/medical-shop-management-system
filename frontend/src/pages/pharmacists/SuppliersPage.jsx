...
import { fetchSuppliers as fetchSuppliersApi } from '../../auth/supplierService';
...
export default function SuppliersPage() {
  ...
  const clientSide = phFilter !== 'all' || monthOnly;
  const LIMIT = 5;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      fetchSuppliersApi(role, params)
        ...
        .catch((err) => {
          if (active) setRowsError(err.response?.data?.message || 'Failed to load suppliers');
        })
        .finally(() => {
  ...
  }, [clientSide, role, page, search, retryTick]);
          ...
          <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-line bg-surface shadow-2xl">
            ...
            {!detailLoading && !detailError && supplierDetail && (() => {
              ...
              const rows = [
                ...
                ...(role === 'admin' ? [{ label: 'Pharmacist', value: displayPharmacistName(s) }] : []),
